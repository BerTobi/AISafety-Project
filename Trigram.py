...
W = torch.randn((vocab_size * 2, len(stoi)), generator=g, requires_grad=True)
...
def training_loss():
    ...
    logits = xenc.view(-1, vocab_size * 2) @ W # predict log-counts
...
for i in range(10):
    ...
    while True:
        ...
        xenc = F.one_hot(torch.tensor((ix1, ix2)), num_classes=len(stoi)).float()
        logits = xenc.view(-1, vocab_size * 2) @ W # predict log-counts
