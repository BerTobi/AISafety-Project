...
def save_model(parameters, itos, filename):
    ...
    torch.save({
        ...
        'b2': b2,
        'itos': itos,
        'context_size': context_size,  # Saving the context_size used during training
        'vocab_size': len(itos)
    }, filename)
...
def load_model_and_sample(filename, num_samples=20, seed=None):
    ...
    for i in range(num_samples):
        ...
        while True:
            emb = C[torch.tensor([context], device=device)]  # (1, context_size, d)
...
save_model([C, W1, b1, W2, b2], itos, "MLP1.w")